...
import nz.co.neo4j.sample.bookshop.data.model.Person;
import nz.co.neo4j.sample.bookshop.data.model.Publication;
...
public class BookNode implements Book {
	...
	private PersonNode asNode(Person person) {
		try {
			...
			throw new IllegalArgumentException("Expected PersonNode instance",
					cce);
		}
	}
	...
	public Person[] getAuthors() {
		Iterable<Relationship> iterables = node
				.getRelationships(BooksRelationship.AuthorOf);
	...
	public void setPublisher(Publisher publisher, int year) {
		...
		Iterable<Relationship> published = node
				.getRelationships(BooksRelationship.PublishedBy);
		for (Relationship r : published) {
			r.delete();
		}

		Relationship newRel = node.createRelationshipTo(pn.node,
				BooksRelationship.PublishedBy);
